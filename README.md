...
3. **Save Font** — writes `<stem>_pua.<ext>` next to the source. Use the toolbar's **Load/Save Profile** to reuse placement settings, or the Controls pane's **Reset Defaults** to start over. Opening a font auto-loads its `<stem>.json` profile when the stamped font identity matches.
