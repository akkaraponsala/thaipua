...
- Offset resolution in `ConsonantSettings.offset_for` layers the per-glyph tiers on top of the base tier: `(combo_offsets[combo_key][role] or mark_offsets[role][mark] or (0,0)) + (base_offsets[role] or (0,0))`; a tone stacked on an above vowel resolves its base tier against the `tone_mark_on_above_vowel` role (falling back to `tone_mark`).
